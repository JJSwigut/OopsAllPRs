...
- The script stores the last successful wireless address in `.env.pixel-adb`; this file is ignored by `.gitignore`.
...
- If the wireless debugging port changes, ask the user for the new main `IP address & port` and run `tools/pixel_adb_wifi.sh connect <ip:port>`.
